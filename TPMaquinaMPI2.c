...
void getCeldasInfectadas(Celda c, Celda *buffer_fila, Celda *buffer_fila_sup, Celda *buffer_fila_arriba, Celda *buffer_fila_abajo,Celda *buffer_fila_inf, int cant_Vecinos, int frontera_fila, int frontera_columna, int *datos_vecino){
    ...
    Celda vecino[12] = {
        buffer_fila_sup[c.cord_y],///arriba j-2
        buffer_fila_arriba[c.cord_y - 1],///arriba j-1, i-1
        buffer_fila_arriba[c.cord_y],///arriba j-1, i
        buffer_fila_arriba[c.cord_y + 1],///arriba j-1, i+1
...
void Calcular_Proximo_Estado(Celda *tablero, Celda *guardo_resultado, Celda c, Celda *busqueda_fila_actual, Celda *buffer_fila_sup, Celda *buffer_fila_arriba, Celda *buffer_fila_abajo, Celda *buffer_fila_inf, int frontera_fila, int frontera_columna){
    ...
    if ((c).estado == VERDE){
        ...
        if (datos_vecinos[1] == 0){    //no hay vecinos enfermos
            ...
            else if ((c).edad >= 157 && (c).edad <= 1820){ //adulto
                susceptibilidad = 0.17;

            }
            else if ((c).edad > 1820){  //viejo
                susceptibilidad = 0.63;

            }
            //Prob. contagio= (%vecinos-con-síntomas + susceptibilidad (edad, heridas))* 0.60 + 0.07
            prob_contagio = (datos_vecinos[1] + susceptibilidad)* 0.6 + 0.07;
        ...
        if((c).timer >= 5){
            ...
            if((c).edad >= 157 && (c).edad <=1820){/// arbol adulto
                if((rand()%100) <= 15){
                    ///aca iria recuperado
                    c.estado = VERDE;
                    c.timer = 0;
            ...
            if((c).edad >=1821){ ///arbol viejo
                if((rand()%100) <= 53){
                    ...
                    //reemplazo por joven sano de un año
                    c.estado = VERDE;
                    c.edad = 52;
                    c.timer = 0;
                }
